...
pub fn parse_request(buf: &[u8]) -> Result<(Request<'_>, usize), ParseError> {
    ...
    let mut space1 = 0;
    while space1 < buf.len() && buf[space1] != b' ' { space1 += 1; }
    if space1 >= buf.len() { return Err(ParseError::Incomplete); }
    let method = Method::from_bytes(&buf[..space1]);

    // Find second space for Path
    let mut space2 = space1 + 1;
    while space2 < buf.len() && buf[space2] != b' ' { space2 += 1; }
    if space2 >= buf.len() { return Err(ParseError::Incomplete); }
    ...
    while req_line_end + 1 < buf.len() && !(buf[req_line_end] == b'\r' && buf[req_line_end+1] == b'\n') {
        req_line_end += 1;
    }
