...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let (client, connection) =
        tokio_postgres::connect("host=127.0.0.1 user=chopin password=chopin dbname=postgres", NoTls).await?;
    ...
    // Setup table
    client.batch_execute("DROP TABLE IF EXISTS bench_tokio").await?;
    client.batch_execute("CREATE TABLE bench_tokio (id INT PRIMARY KEY, val TEXT, count INT)").await?;
    ...
    for scale in scales {
        ...
        for i in 0..10_000 {
            ...
            let _ = client.query_one("SELECT val FROM bench_tokio WHERE id = $1", &[&id]).await?;
        }
        ...
        for i in 0..10_000 {
            let id = (i % scale) as i32;
            let _ = client.execute("UPDATE bench_tokio SET count = count + 1 WHERE id = $1", &[&id]).await?;
        }
        let duration = start.elapsed();
        ...
        for i in 0..10_000 {
            ...
            let _ = client.execute("INSERT INTO bench_tokio (id, val, count) VALUES ($1, $2, $3)", &[&id, &"new", &0i32]).await?;
        }
        let duration = start.elapsed();
