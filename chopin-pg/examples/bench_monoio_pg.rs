...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    client.execute("CREATE TABLE bench_mono (id INT PRIMARY KEY, val TEXT, count INT)").await?;
    ...
    for scale in scales {
        ...
        for i in 0..(scale / 100).max(1) {
             ...
             for j in 0..100 {
                 let id = i * 100 + j;
                 if id >= scale { break; }
                 sql.push_str(&format!("({}, 'val_{}', {}){}", id, id, id, if j == 99 || i*100+j == scale-1 { "" } else { "," }));
             }
        ...
        for i in 0..10_000 {
            let id = i % scale;
            let _ = client.query(&format!("SELECT val FROM bench_mono WHERE id = {}", id)).await?;
        }
        ...
        for i in 0..10_000 {
            let id = i % scale;
            let _ = client.execute(&format!("UPDATE bench_mono SET count = count + 1 WHERE id = {}", id)).await?;
        }
        let duration = start.elapsed();
        ...
        for i in 0..10_000 {
            let id = scale + i;
            let _ = client.execute(&format!("INSERT INTO bench_mono (id, val, count) VALUES ({}, 'new', 0)", id)).await?;
        }
        let duration = start.elapsed();
